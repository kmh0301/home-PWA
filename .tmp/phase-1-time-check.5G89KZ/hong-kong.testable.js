import { getHongKongDayBounds, getHongKongMonthBounds, getHongKongWeekBounds, toHongKongDateParts, } from "./hong-kong";
...
export function verifyHongKongBoundaryExpectations() {
    return BOUNDARY_EXPECTATIONS.map((expectation) => {
        ...
        assertEqual(`${expectation.label} date key`, dateParts.dateKey, expectation.expectedDateKey);
        assertEqual(`${expectation.label} day start`, dayBounds.start.toISOString(), expectation.expectedDayStart);
        assertEqual(`${expectation.label} day end`, dayBounds.end.toISOString(), expectation.expectedDayEnd);
        assertEqual(`${expectation.label} week start`, weekBounds.start.toISOString(), expectation.expectedWeekStart);
        assertEqual(`${expectation.label} week end`, weekBounds.end.toISOString(), expectation.expectedWeekEnd);
        assertEqual(`${expectation.label} month start`, monthBounds.start.toISOString(), expectation.expectedMonthStart);
        assertEqual(`${expectation.label} month end`, monthBounds.end.toISOString(), expectation.expectedMonthEnd);
        return {
